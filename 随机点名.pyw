...
# 尝试读取姓名和概率数据文件
try:
    with open("姓名及概率.txt", encoding='utf-8') as 姓名及概率_csv:
        姓名及概率 = 姓名及概率_csv.read()
        ...
        # 验证文件格式（第一行应为标题）
        if 姓名及概率[0] != "姓名,概率":
            raise NameError("数据文件格式错误：第一行应为'姓名,概率'")

        # 解析数据行，提取姓名和概率
        for 单人姓名及概率 in 姓名及概率[1:]:
            if 单人姓名及概率.strip():  # 跳过空行
                姓名列表.append((单人姓名及概率.split(","))[0])           # 提取姓名
                概率列表.append(int(float((单人姓名及概率.split(","))[1])))  # 提取概率并转为整数
except:
    当前名字 = "未找到文件"
...
def 关闭():
    ...
    for 帧数 in range(启动_关闭帧数):
        窗口.wm_attributes("-alpha", 透明度 - (透明度 / 启动_关闭帧数) * 帧数) # 设置透明度(递减)
        窗口.update()
    窗口.quit()
...
def 初始化():
    ...
    for 值 in 概率列表:
        索引 = 0
        for 当前项2 in list(str(值)):
            索引 += 1
            if 当前项2==".":
                if len(list(str(值)))-索引>浮点位数:
                    浮点位数 = len(list(str(值))) - 索引
                break
    # 将概率转换为整数（避免浮点数精度问题）
    浮点位数 = 10 ** 浮点位数
    for 索引 in range(len(概率列表)):
        概率列表[索引] = 概率列表[索引] * 浮点位数
...
def 随机函数(传入姓名列表, 传入概率列表):
    ...
    for 当前项 in 传入概率列表:
        累计概率 += 当前项
        if 累计概率>=当前概率数 and 当前概率数>=前一个累计概率:
...
def 随机():
    """根据当前模式进行随机选择"""
    global 当前模式, 当前名字, 临时姓名列表, 姓名列表, 临时概率列表, 概率列表
...
def 查看概率():
    ...
    临时变量 = ""
    # 格式化显示概率信息
    for i in range(len(概率列表)):
        临时变量 += f"{姓名列表[i]}的概率为{概率列表[i]} "
        if (i+1)%5 == 0:  # 每5个姓名换行
            临时变量 += "\n"
    名字标签.config(text=临时变量, font=名字字体)
...
def 隐藏(动画=True):
    ...
    if 隐藏状态:
        ...
        if 动画 and not 窗口化状态:
            ...
            # 启动动画
            for 帧数 in range(启动_关闭帧数):
                窗口.wm_attributes("-alpha", (透明度 / 启动_关闭帧数) * 帧数) # 设置透明度(递增)
                窗口.update()
...
def 更多选项显示():
    ...
    文件路径文本.place(x=10, y=10, anchor="nw")
...
字体大小滑块 = t.Scale(窗口, from_=10, to=90, orient="horizontal", command=更新字体大小, showvalue=False)
字体大小滑块.set(名字字体[1])  # 设置滑块初始值
窗口化状态 = False
窗口化按钮 = t.Button(窗口, text="窗口化", command=窗口化)
